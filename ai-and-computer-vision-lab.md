...
2. In the Arduino IDE, open **Tools → Manage Libraries**, search **FastLED**,
   and install it. The sketch needs this library to build.
3. Upload `xiao/xiao.ino` again.

Drawing never talks to the LEDs directly. Every drawing call just changes an
in-memory 16×16 buffer; `renderMatrix()` is what pushes that buffer to the real
panel, in two steps:

- `lightGrid.show()` walks the grid and hands each pixel's `(x, y, color)` to the
  `matrixWriter` function.
- `matrixWriter` turns that grid position into the right LED number — undoing the
  zig-zag (serpentine) wiring of most panels — and `FastLED.show()` lights them.

All of that is written for you in `xiao.ino` (the `matrixWriter` and
`renderMatrix` functions). You just draw on the grid.
...
#### Section: Draw Your Own Picture in the Browser

Shapes are fun, but the real payoff is drawing your **own picture** — in full
color, without typing out 256 numbers by hand. The lab website has a drawing tool
that does the bookkeeping for you, and the firmware is already set up to show
whatever you draw.
...
3. Click **Copy code**. The firmware has a `MY_ART` array near the top of
   `xiao/xiao.ino` that starts **blank** (`= {0}`, all pixels off) — **replace that
   whole line** with the snippet you just copied:

```cpp
// Pixel art — generated by the lab's Draw tool. Replaces the blank MY_ART.
const uint32_t MY_ART[NUM_LEDS] = {
  0x000000, 0xFF0000, 0xFF0000, /* ...256 colors, one per pixel... */ 0x0000FF,
};
```

The array is **256 numbers** (`NUM_LEDS` is `16 * 16`) — one `0xRRGGBB` color per
pixel, reading left to right across each row, top row first (`index = y * 16 + x`).
The example draws it with `drawFrame`, which copies all 256 straight onto the grid:
...
Upload, and your art shows on boot and whenever you make the **open-palm** gesture
(see "Wire It to Your Gestures" below). Re-draw in the browser, copy, replace the
array, and upload again to change the picture.

> For a quick **one-color** icon there's also `drawBitmap(rows, height, color)` —
> each row is a 16-bit number and every `1` bit lights a pixel. The browser tool
> is easier and gives you full color, so reach for `drawFrame` first.

#### Section: Paste Your Drawing Into the Sketch

Here's exactly where the snippet goes. Open `xiao/xiao.ino` and find this line
near the top (just above `void addProjectStatus(...)`):

```cpp
const uint32_t MY_ART[NUM_LEDS] = {0};   // <-- blank canvas
```

Select that whole line, delete it, and paste the **Copy code** snippet from the
Draw tool in its place:

```cpp
const uint32_t MY_ART[NUM_LEDS] = {
  0x000000, 0xFF0000, /* ...256 colors from the Draw tool... */ 0x0000FF,
};
```

Click **→ Upload**. Nothing else changes — `setup()` and the `open_hand` gesture
already call `drawFrame(MY_ART)`, so your picture shows on boot and on an open
palm. To change it, re-draw in the browser and paste over the array again.
...
The example firmware already wires the grid to two gestures. An **open palm**
draws your art (`MY_ART`); a **fist** (or the `off` command) clears it. (Your art
also shows on boot — `setup()` calls the same `drawFrame` + `renderMatrix`.)
...
Each `if` maps one command to one action. Remember `renderMatrix()` — the grid
calls only change the in-memory buffer until you push it to the panel.

Make it yours: change `MY_ART` by drawing again in the browser, play an animation
in a `for` loop instead (see above), or add another `if` block for a second
gesture.
...
- Nothing, any board works as-is
- *Wire the panel to D6 and install the FastLED library, then upload
- A second XIAO board
